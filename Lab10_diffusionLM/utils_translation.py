...
class TranslationDataset(Dataset):
    def __init__(self, raw_dataset, tokenizer, seq_len: int):
        self.seq_len = seq_len
        self.mn_data = []
        self.en_data = []

        for mn_sent, en_sent in tqdm(zip(raw_dataset["sentence"], raw_dataset["sentence_en"]), total=len(raw_dataset["sentence"])):
           
            mn_encoded = tokenizer(mn_sent, max_length=seq_len, padding="max_length", truncation=True, return_tensors="pt")
            en_encoded = tokenizer(en_sent, max_length=seq_len, padding="max_length", truncation=True, return_tensors="pt")
            
            self.mn_data.append(mn_encoded.input_ids.squeeze(0))
            self.en_data.append(en_encoded.input_ids.squeeze(0))
    
    def __len__(self):
        return len(self.mn_data)

    def __getitem__(self, idx):
         return self.mn_data[idx], self.en_data[idx]
...
def diffusion_loss(model, x0, alphas_bar, cfg):
    ...
    is_not_pad = (x0 != cfg.pad_token_id)
    ...
    return masked_loss, num_masked_tokens.item()
